...
template<typename Element, typename SmemTensor>
__forceinline__ __device__ void copy_int4_kv_to_smem(
        const uint8_t *__restrict__ packed_ptr,
        const float *__restrict__ scale_ptr,
        SmemTensor &sKV,
        const int64_t packed_row_stride,
        const int64_t scale_row_stride,
        const int64_t scale_group_stride,
        const int head_dim,
        const int group_size,
        const int max_rows,
        const bool clear_oob_rows) {
    constexpr int kRows = decltype(size<0>(sKV))::value;
    constexpr int kCols = decltype(size<1>(sKV))::value;
    #pragma unroll
    for (int linear = threadIdx.x; linear < kRows * kCols; linear += blockDim.x) {
        const int row = linear / kCols;
        const int col = linear - row * kCols;
        const bool valid = row < max_rows && col < head_dim;
        float value = 0.0f;
        if (valid) {
            const uint8_t byte = packed_ptr[row * packed_row_stride + (col >> 1)];
            const int nibble = (col & 1) == 0 ? (byte & 0x0f) : ((byte >> 4) & 0x0f);
            const int signed_value = nibble >= 8 ? nibble - 16 : nibble;
            const float scale = scale_ptr[row * scale_row_stride + (col / group_size) * scale_group_stride];
            value = float(signed_value) * scale;
        }
        if (valid || clear_oob_rows) {
            sKV(row, col) = Element(value);
        }
    }
}
...
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    const index_t row_offset_ks = block_table == nullptr
        ? binfo.k_offset(params.k_scale_batch_stride, params.k_scale_row_stride, bidb_cache)
          + (n_block_max - 1) * kBlockN * params.k_scale_row_stride
          + (bidh / params.h_h_k_ratio) * params.k_scale_head_stride
        : block_table[block_table_idx] * params.k_scale_batch_stride
          + block_table_offset * params.k_scale_row_stride
          + (bidh / params.h_h_k_ratio) * params.k_scale_head_stride;
    const index_t row_offset_vs = block_table == nullptr
        ? binfo.k_offset(params.v_scale_batch_stride, params.v_scale_row_stride, bidb_cache)
          + (n_block_max - 1) * kBlockN * params.v_scale_row_stride
          + (bidh / params.h_h_k_ratio) * params.v_scale_head_stride
        : block_table[block_table_idx] * params.v_scale_batch_stride
          + block_table_offset * params.v_scale_row_stride
          + (bidh / params.h_h_k_ratio) * params.v_scale_head_stride;
    const uint8_t *k_int4_ptr = reinterpret_cast<const uint8_t *>(params.k_ptr) + row_offset_k;
    const uint8_t *v_int4_ptr = reinterpret_cast<const uint8_t *>(params.v_ptr) + row_offset_v;
    const float *k_scale_ptr = reinterpret_cast<const float *>(params.k_scale_ptr) + row_offset_ks;
    const float *v_scale_ptr = reinterpret_cast<const float *>(params.v_scale_ptr) + row_offset_vs;
    ...
    int n_block = n_block_max - 1;
    // We don't need to clear the sK smem tiles since we'll mask out the scores anyway.
    if (params.is_kv_int4) {
        FLASH_NAMESPACE::copy_int4_kv_to_smem<Element>(
            k_int4_ptr,
            k_scale_ptr,
            sK,
            params.k_row_stride,
            params.k_scale_row_stride,
            params.k_scale_group_stride,
            params.d,
            params.k_int4_group_size,
            binfo.actual_seqlen_k - n_block * kBlockN,
            true
        );
    } else {
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (masking_step > 0) {
            if (block_table == nullptr) {
                tVgV.data() = tVgV.data() + (-int(kBlockN * params.v_row_stride));
                v_int4_ptr += -int(kBlockN * params.v_row_stride);
                v_scale_ptr += -int(kBlockN * params.v_scale_row_stride);
            } else {
                ...
                v_int4_ptr += (block_table[block_table_idx_next] - block_table[block_table_idx_cur]) * params.v_batch_stride + (block_table_offset_next - block_table_offset_cur) * params.v_row_stride;
                v_scale_ptr += (block_table[block_table_idx_next] - block_table[block_table_idx_cur]) * params.v_scale_batch_stride + (block_table_offset_next - block_table_offset_cur) * params.v_scale_row_stride;
            }
            if (params.is_kv_int4) {
                FLASH_NAMESPACE::copy_int4_kv_to_smem<Element>(
                    v_int4_ptr, v_scale_ptr, sV, params.v_row_stride, params.v_scale_row_stride,
                    params.v_scale_group_stride, params.d, params.k_int4_group_size, kBlockN, false
                );
            } else {
        ...
        if (n_block > n_block_min) {
            // Advance gK
                if (block_table == nullptr) {
                tKgK.data() = tKgK.data() + (-int(kBlockN * params.k_row_stride));
                k_int4_ptr += -int(kBlockN * params.k_row_stride);
                k_scale_ptr += -int(kBlockN * params.k_scale_row_stride);
            } else {
                const int block_table_idx_cur = n_block * kBlockN / params.page_block_size;
                const int block_table_offset_cur = n_block * kBlockN - block_table_idx_cur * params.page_block_size;
                const int block_table_idx_next = (n_block - 1) * kBlockN / params.page_block_size;
                ...
                k_int4_ptr += (block_table[block_table_idx_next] - block_table[block_table_idx_cur]) * params.k_batch_stride + (block_table_offset_next - block_table_offset_cur) * params.k_row_stride;
                k_scale_ptr += (block_table[block_table_idx_next] - block_table[block_table_idx_cur]) * params.k_scale_batch_stride + (block_table_offset_next - block_table_offset_cur) * params.k_scale_row_stride;
            }
            if (params.is_kv_int4) {
                FLASH_NAMESPACE::copy_int4_kv_to_smem<Element>(
                    k_int4_ptr, k_scale_ptr, sK, params.k_row_stride, params.k_scale_row_stride,
                    params.k_scale_group_stride, params.d, params.k_int4_group_size, kBlockN, false
                );
            } else {
