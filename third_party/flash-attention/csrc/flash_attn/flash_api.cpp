...
mha_fwd_kvcache_int4(at::Tensor &q,
                     const at::Tensor &kcache_packed,
                     const at::Tensor &vcache_packed,
    ...
    auto [cc_major, cc_minor] = get_compute_capability(get_current_device());
    TORCH_CHECK(cc_major >= 8, "FlashAttention INT4 KV-cache only supports Ampere GPUs or newer.");

    auto q_dtype = q.dtype();
    TORCH_CHECK(q_dtype == torch::kFloat16 || q_dtype == torch::kBFloat16,
                "FlashAttention INT4 KV-cache only supports fp16 and bf16 query dtype");
    TORCH_CHECK(kcache_packed.dtype() == torch::kUInt8, "INT4 key cache must be packed uint8");
    TORCH_CHECK(vcache_packed.dtype() == torch::kUInt8, "INT4 value cache must be packed uint8");
    TORCH_CHECK(k_scale.dtype() == torch::kFloat32, "INT4 key scales must be float32");
    TORCH_CHECK(v_scale.dtype() == torch::kFloat32, "INT4 value scales must be float32");
    CHECK_DEVICE(q); CHECK_DEVICE(kcache_packed); CHECK_DEVICE(vcache_packed); CHECK_DEVICE(k_scale); CHECK_DEVICE(v_scale);
    TORCH_CHECK(q.stride(-1) == 1, "Query tensor must have contiguous last dimension");
    TORCH_CHECK(kcache_packed.stride(-1) == 1, "Packed key cache must have contiguous last dimension");
    TORCH_CHECK(vcache_packed.stride(-1) == 1, "Packed value cache must have contiguous last dimension");
    TORCH_CHECK(k_scale.stride(-1) == 1, "Key scale tensor must have contiguous last dimension");
    TORCH_CHECK(v_scale.stride(-1) == 1, "Value scale tensor must have contiguous last dimension");
    ...
    const int num_heads = sizes[2];
    const int head_size = sizes[3];
    TORCH_CHECK(seqlen_q == 1, "INT4 KV-cache FlashAttention path currently supports decode q_len=1 only");
    TORCH_CHECK(head_size % 8 == 0, "INT4 KV-cache head_dim must be divisible by 8");
    TORCH_CHECK(group_size > 0 && head_size % group_size == 0, "INT4 group_size must divide head_dim");

    const int num_heads_k = kcache_packed.size(1);
    const int seqlen_k = kcache_packed.size(2);
    TORCH_CHECK(kcache_packed.size(0) == batch_size, "INT4 KV-cache batch size must match query batch size");
    TORCH_CHECK(num_heads % num_heads_k == 0, "Number of query heads must be divisible by number of KV heads");
    CHECK_SHAPE(kcache_packed, batch_size, num_heads_k, seqlen_k, head_size / 2);
    CHECK_SHAPE(vcache_packed, batch_size, num_heads_k, seqlen_k, head_size / 2);
    CHECK_SHAPE(k_scale, batch_size, num_heads_k, seqlen_k, head_size / group_size);
    CHECK_SHAPE(v_scale, batch_size, num_heads_k, seqlen_k, head_size / group_size);

    is_causal = false;  // q_len=1 decode over a used KV prefix is equivalent to non-causal attention.
    if (window_size_left >= seqlen_k) { window_size_left = -1; }
    if (window_size_right >= seqlen_k) { window_size_right = -1; }
    ...
    params.k_ptr = kcache_packed.data_ptr();
    params.v_ptr = vcache_packed.data_ptr();
    params.k_batch_stride = kcache_packed.stride(0);
    params.v_batch_stride = vcache_packed.stride(0);
    params.k_head_stride = kcache_packed.stride(1);
    params.v_head_stride = vcache_packed.stride(1);
    params.k_row_stride = kcache_packed.stride(2);
    params.v_row_stride = vcache_packed.stride(2);

    params.is_kv_int4 = true;
    ...
    params.k_scale_group_stride = k_scale.stride(3);
    params.v_scale_group_stride = v_scale.stride(3);
    params.k_int4_group_size = group_size;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("fwd_kvcache_int4", &FLASH_NAMESPACE::mha_fwd_kvcache_int4, "Forward pass with packed INT4 KV-cache");
}
