...
def _quantize_initial_state(state: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    if state.shape[-1] % MX8_BLOCK_SIZE != 0:
        raise ValueError(f"MX8 state dim must be divisible by {MX8_BLOCK_SIZE}, got {state.shape[-1]}")
    working = state.float().contiguous()
    blocks = working.reshape(*working.shape[:-1], working.shape[-1] // MX8_BLOCK_SIZE, MX8_BLOCK_SIZE)
    amax = blocks.abs().amax(dim=-1, keepdim=True)
    raw_exp = torch.ceil(torch.log2(torch.clamp(amax / 126.0, min=1.0e-30)))
    ...
    pairs = blocks.reshape(*blocks.shape[:-1], MX8_BLOCK_SIZE // MX8_PAIR_SIZE, MX8_PAIR_SIZE)
    pair_amax = pairs.abs().amax(dim=-1, keepdim=True)
    micro = (pair_amax > MX8_MANTISSA_MAX * base_scale.unsqueeze(-2)).to(torch.uint8)
    pair_scale = base_scale.unsqueeze(-2) * torch.exp2(micro.float())
    q = torch.round(torch.clamp(pairs.abs() / pair_scale, 0.0, MX8_MANTISSA_MAX)) * pairs.sign()
    q_state = q.reshape_as(working).to(torch.int8)
    shared_exp = biased_exp.squeeze(-1).to(torch.uint8).contiguous()
    micro_exp = micro.squeeze(-1).reshape(*working.shape[:-1], working.shape[-1] // MX8_PAIR_SIZE).contiguous()
...
def allocate_state_kernel_cache(ssm_state_4d: torch.Tensor, group_size: int = MX8_BLOCK_SIZE, format_bits: int = 8) -> MambaStateKernelCache:
    if format_bits != 8:
        raise ValueError("Only MX8 state quantization is supported in this cleaned kernel path.")
    if group_size != MX8_BLOCK_SIZE:
        raise ValueError(f"MX8 uses a fixed {MX8_BLOCK_SIZE}-value block, got group_size={group_size}")
    q_state, shared_exp, micro_exp = _quantize_initial_state(ssm_state_4d)
    return MambaStateKernelCache(q_state=q_state, shared_exp=shared_exp, micro_exp=micro_exp)


def mx8_state_selective_update(
...
def patch_nemotron_h_mamba_decode_state_kernel(
    ...
    if getattr(module, "_mx8_state_kernel_patched", False):
        return

    def selective_state_update_mx8(ssm_state, x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False):
        ...
        return mx8_state_selective_update(
            kernel_cache,
            x,
            dt,
            A,
    ...
    module.selective_state_update = selective_state_update_mx8
    module._mx8_state_kernel_original = original
    module._mx8_state_kernel_patched = True
    model._mx8_state_kernel_group_size = group_size
...
def register_mamba_state_kernel_caches(model, cache_params, mode_by_layer: dict[int, str]) -> None:
    ...
    for layer_idx, mode in mode_by_layer.items():
        if mode != "mx8":
            raise ValueError(f"Only MX8 state caches are supported, got mode={mode!r}")
        ...
        caches[id(ssm_state)] = allocate_state_kernel_cache(state_4d, group_size=MX8_BLOCK_SIZE, format_bits=8)
    model._mamba_decode_state_kernel_caches = caches
    model._mamba_decode_state_kernel_meta = {
        layer_idx: SimpleNamespace(mode=mode_by_layer[layer_idx], block_size=MX8_BLOCK_SIZE)
