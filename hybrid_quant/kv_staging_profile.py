...
class KVStagingProfiler:
    ...
    def start_step(self, step_idx: int, decode_position: int, cache_params, device: torch.device) -> None:
        ...
        if self.staging_mode == "dequant":
            ...
            self.schedule_stage(self.attention_layer_ids[0], self.step_start_event)
        if self.enable_nvtx:
            self.nvtx_push(f"gap_before_attention_{self.attention_layer_ids[0]}")

    def end_step(self) -> None:
        if self.enable_nvtx:
            # Close the final post-attention gap range if no later attention closed it.
            try:
                self.nvtx_pop()
            except RuntimeError:
                pass
    ...
    def temp_buffers(self, attn_layer_id: int, layer_cache, seq_len: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if current is not None:
            tmp_k, tmp_v = current
            if tmp_k.shape[0] == batch and tmp_k.shape[1] == kv_heads and tmp_k.shape[2] >= seq_len:
                return tmp_k, tmp_v
        tmp_k = torch.empty((batch, kv_heads, seq_len, head_dim), device=layer_cache.k_packed.device, dtype=dtype)
        tmp_v = torch.empty((batch, kv_heads, seq_len, head_dim), device=layer_cache.k_packed.device, dtype=dtype)
        self.tmp_buffers[attn_layer_id] = (tmp_k, tmp_v)
        return tmp_k, tmp_v

    def schedule_stage(self, attn_layer_id: int | None, dependency: torch.cuda.Event | None) -> None:
        ...
        with torch.cuda.stream(stream):
            ...
            if self.enable_nvtx:
                torch.cuda.nvtx.range_push(f"stage_kv_for_attention_{attn_layer_id}")
            start.record(stream)
            dequant_int4_cache_to_temp(layer_cache, seq_len, tmp_k, tmp_v, self.kv_group_size)
            end.record(stream)
            if self.enable_nvtx:
                torch.cuda.nvtx.range_pop()
        self.scheduled_stages[attn_layer_id] = StageRecord(
    ...
    def before_attention_layer(self, attn_layer_id: int) -> tuple[torch.cuda.Event, EventSpan | None, StageRecord | None]:
        ...
        if stage is not None:
            ...
            torch.cuda.current_stream(self.device).wait_event(stage.end)
    ...
    def attention_mixer(self, attn_layer_id: int, fn, *args, **kwargs):
        ...
        attn_end = self.event()
        self.nvtx_push(f"attention_{attn_layer_id}")
        ...
        self.schedule_stage(next_attn, attn_end)
        if self.enable_nvtx and next_attn is not None:
            self.nvtx_push(f"gap_before_attention_{next_attn}")
        return out

    def attention_rows(self, gap_slowdown_by_step: dict[tuple[int, int], float | None] | None = None) -> list[dict]:
        rows = []
        for rec in self.attention_records:
            ...
            hideability = None if not stage_ms else rec.gap.elapsed_ms() / stage_ms
            ...
            rows.append(
                {
                    ...
                    "gap_slowdown_pct": None
                    if gap_slowdown_by_step is None
                    else gap_slowdown_by_step.get((rec.step_idx, rec.attn_layer_id)),
                }
            )
        return rows
